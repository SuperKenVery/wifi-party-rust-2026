...
use cubecl::frontend::{CompilationArg, CubeIndexExpand, CubeIndexMutExpand, ExpExpand, TanhExpand};
use cubecl::prelude::{AddressType, CubeDim, Float, ReadWrite, StorageType, ABSOLUTE_POS, terminate};
use cubecl::std::tensor::layout::linear::LinearView;

type WgpuCubeBackend = CubeBackend<WgpuRuntime, f32, i32, u32>;

pub(super) trait CustomLstmBackend:
    Backend<
        Device = WgpuDevice,
        FloatElem = f32,
...
fn cube_tensor<B: CustomLstmBackend, const D: usize>(tensor: Tensor<B, D>) -> CubeTensor<WgpuRuntime> {
    tensor.into_primitive().tensor()
}

fn burn_tensor<B: CustomLstmBackend, const D: usize>(tensor: CubeTensor<WgpuRuntime>) -> Tensor<B, D> {
...
fn launch_lstm_step_f32(
    ...
    let address_type = lstm_step_address_type(&[
        x_proj, w_h, b_h, h_prev, c_prev, h_next, c_next, output,
    ]);
...
fn lstm_preproj<B: CustomLstmBackend>(
    lstm: &burn::nn::Lstm<B>,
    input: Tensor<B, 3>,             // [seq, batch, input_size]
    state: Option<LstmState<B, 2>>,  // h/c each [batch, hidden]
) -> (Tensor<B, 3>, LstmState<B, 2>) {
...
#[cfg(test)]
pub(super) fn lstm_preproj_equivalence_error<B: CustomLstmBackend>(device: &B::Device) -> f32 {
    let seq = 5;
    let batch = 3;
    let input_size = 16;
    let hidden = 32;
