...
impl AudioCaptureHandler {
        ...
        let stream = device
            .build_input_stream(
                ...
                move |data: &[T], _: &cpal::InputCallbackInfo| {
                    if let Err(e) = Self::process_input_data(
                        data,
                        &mut buffer,
                        &state,
                        &mut network_producer,
                        &mut loopback_producer,
                        frame_size,
                    ) {
                        error!("Error processing input data: {}", e);
                    }
                },
                move |err| {
    ...
    fn process_input_data<T>(
        ...
        buffer: &mut Vec<i16>,
        state: &AppState,
        network_producer: &mut Producer<Vec<u8>>,
        loopback_producer: &mut Producer<Vec<i16>>,
        frame_size: usize,
    ) -> Result<()>
    where
        T: cpal::Sample + cpal::SizedSample,
    {
        ...
        for sample in data {
            ...
            if buffer.len() >= frame_size {
                ...
                // Create audio frame (no host_id, sample_rate is always 48kHz, channels is always 2)
                let frame =
                    AudioFrame::new(seq, samples).context("Failed to create audio frame")?;

                // Serialize frame
                let serialized = frame.serialize().context("Failed to serialize frame")?;
